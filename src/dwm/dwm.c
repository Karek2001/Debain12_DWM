...
drawbar(Monitor *m)
{
        ...
        int mstext_width = 0;
        int rstext_width = 0;
        int padding = 10; /* Space between center and right text */
        ...
        if (m == selmon) { /* status is only drawn on selected monitor */
                ...
                if (splitstatus) {
                        ...
                        mstext_width = TEXTW(mstext) - lrpad;  /* Center text width */
                        rstext_width = TEXTW(rstext) - lrpad;  /* Right text width */

                        /* Make sure center text does not overlap with right text */
                        if (m->ww - rstext_width - padding > mstext_width) {
                                msx = (m->ww - mstext_width - rstext_width - padding) / 2;
                        } else {
                                msx = (m->ww - mstext_width) / 2; /* Fallback */
                        }

                        drw_text(drw, msx, 0, mstext_width, bh, 0, mstext, 0);
                }

                /* Right-aligned text */
                tw = rstext_width + 4; /* 4px right padding */
